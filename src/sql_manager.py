...
class instance:
    ...
    def insert_or_update_message(self, message, force_chat_id:str=None) -> None:
        ...
        if cursor.execute("SELECT id FROM message WHERE id=?", (message.message_id,)).fetchone():
            ...
            (message.get_chat().chat_id, message_author, message.model, message.dt.strftime("%Y/%m/%d %H:%M:%S"), message.text, message.message_id))
        else:
            cursor.execute("INSERT INTO message (id, chat_id, role, model, date_time, content) VALUES (?, ?, ?, ?, ?, ?)",
            (message.message_id, message.get_chat().chat_id, message_author, message.model, message.dt.strftime("%Y/%m/%d %H:%M:%S"), message.text))
        sqlite_con.commit()
        sqlite_con.close()
