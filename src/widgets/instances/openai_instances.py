...
class Sarvam(BaseInstance):
    ...
    description = _('Sarvam AI large language models')

    def start(self):
        if not self.client:
            # inject the custom API-Subscription-Key header required by Sarvam
            self.client = openai.OpenAI(
                api_key=self.properties.get('api'), # satisfy the OpenAI client requirements ref sarvam docs
                base_url=self.properties.get('url').strip(),
                default_headers={
                    "API-Subscription-Key": self.properties.get('api')
                }
            )
